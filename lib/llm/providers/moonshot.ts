import { LLMProviderClient, LLMMessage } from '../types'

export class MoonshotProvider implements LLMProviderClient {
    private baseUrl: string
    private apiKey: string
    private defaultModel: string

    constructor() {
        this.baseUrl = process.env.MOONSHOT_BASE_URL || 'https://api.moonshot.cn/v1'
        this.apiKey = process.env.MOONSHOT_API_KEY || ''
        this.defaultModel = process.env.MOONSHOT_DEFAULT_MODEL || 'moonshot-v1-8k'
    }

    async generateResponse(messages: LLMMessage[], options: { model?: string }): Promise<string> {
        if (!this.apiKey) {
            throw new Error('Moonshot API key not configured')
        }

        const model = options.model || this.defaultModel

        try {
            const response = await fetch(`${this.baseUrl}/chat/completions`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${this.apiKey}`,
                },
                body: JSON.stringify({
                    model,
                    messages,
                    temperature: 0.7,
                    max_tokens: 1000,
                }),
            })

            if (!response.ok) {
                const error = await response.json().catch(() => ({ error: 'Unknown error' }))
                throw new Error(`Moonshot API error: ${response.status} - ${JSON.stringify(error)}`)
            }

            const data = await response.json()
            return data.choices?.[0]?.message?.content || ''
        } catch (error) {
            console.error('Moonshot generation error:', error)
            throw error
        }
    }
}

export const moonshotProvider = new MoonshotProvider()
