...
[
  ...
  ...
        ...
        if (saveToDb && userId) {
            ...
            // 1. Prepare data array for BULK INSERT
            ...
            // 2. BULK INSERT - One shot, all messages
            const { data, error } = await supabaseAdmin
                .from('messages')
                .insert(messagesToInsert)
                .select() // Important: Request the inserted data back

            if (error) {
                console.error('[DEV CHAT GEN] Bulk insert error:', error)
